...
public class Workshop{
    ...
    private ArrayList<at.altin.fh.lpw.workshopexample.Worker> workerList = new ArrayList<>();
    private Job job;
    public void addWorker(at.altin.fh.lpw.workshopexample.Worker worker) {
    ...
    private int getPrice() {
        int price = 0;
        for (at.altin.fh.lpw.workshopexample.Worker worker: workerList){
    ...
    public void printReport(){
        ...
        for (Worker worker: workerList){
            System.out.printf("- %s %d x %d%n", worker.toString(), job.getHours() / workerList.size(), worker.getHourlyRate());
        }
